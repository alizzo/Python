def box():              #this creates a function called box -- I learned it from the text book written by John Zelle in chapter 1. After this program has been run - the user can call the function "box()" to re-reun the code - it's helpful in the debugging stages
    ...
    mid = float(CountValue // 2)     # (also you can use // to divide intergers & it will discard remainder --- The variable "mid" stores one half of the user input as a floating point integer less .5 -- this variable is only call in the case where the user input is odd -- this statement helps to determine the mid point. 
